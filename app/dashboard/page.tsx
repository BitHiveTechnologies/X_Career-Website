...
export default function Page() {
    ...
    useEffect(() => {
        void (
            /* console.log */ ((..._args) => {})('🔍 Dashboard: Auth state changed', {
                user: user?.email,
                role: user?.role,
                authLoading,
            })
        ...
        if (!authLoading && user && (user.role === 'admin' || user.role === 'super_admin')) {
            void (
                /* console.log */ ((..._args) => {})(
                    '🔍 Dashboard: Starting data fetch (bulletproof mode)...',
                )
            );
            fetchDashboardData();
            ...
            void (
                /* console.log */ ((..._args) => {})(
                    '🔍 Dashboard: Still loading auth state or not admin',
                )
            );
        }
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            try {
                ...
                void (
                    /* console.error */ ((..._args) => {})('❌ Payments API failed:', paymentsError)
                );
            }
            ...
            try {
                void (
                    /* console.log */ ((..._args) => {})(
                        '🔍 Fetching customers with working token...',
                    )
                );
            ...
            try {
                ...
                if (jobsResponse.success && jobsResponse.data && jobsResponse.data.jobs) {
                    ...
                    toast.success(
                        `Refreshed data: ${transformedJobs.length} jobs, ${transformedInternships.length} internships`,
                    );
                }
            } catch (apiError) {
    ...
    const fetchAlertStats = async () => {
        try {
            ...
            void (
                /* console.error */ ((..._args) => {})('Error fetching alert statistics:', error)
            );
        }
    };
...
• Duplicates Skipped: ${stats.duplicateNotifications ?? 0}`);
                ...
                toast.error(
                    `Failed to send alerts for "${jobTitle}": ${response.error?.message || 'Unknown error'}`,
                );
            }
        } catch (error: any) {
    ...
    const handleStopAlerts = async () => {
        try {
            ...
            if (res.success) {
                toast.info(
                    res.data?.stopRequested
                        ? 'Stopping — the batch will halt after the current email.'
...
• Duplicates Skipped: ${d.totalDuplicates ?? 0}`);
                ...
                toast.error(
                    `Failed to send all job alerts: ${response.error?.message || 'Unknown error'}`,
                );
            }
        } catch (error: any) {
    ...
    const handleQuickCreate = async (formData: any) => {
        try {
            ...
            const jobData = {
                ...
                ...(formData.type === 'internship' && formData.stipend
                    ? { stipend: formData.stipend }
                    : {}), // Only include stipend for internships
            ...
            if (response.success) {
                toast.success(
                    `${formData.type === 'job' ? 'Job' : 'Internship'} created successfully!`,
                );
    ...
    return (
        <SharedLayout>
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
                ...
                <SectionCards stats={dashboardStats?.overview} growth={dashboardStats?.growth} />
                <div className="px-4 lg:px-6">
                            ...
                            <div className="mt-4 bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                                ...
                                <div className="h-2 w-full rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
                                    <div
                                        ...
                                        style={{
                                            width: `${batchProgress.total ? Math.min(100, Math.round((batchProgress.processed / batchProgress.total) * 100)) : 0}%`,
                                        }}
                                    />
                                </div>
                                <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs">
                                    <span className="text-green-600 dark:text-green-400">
                                        ✓ Sent:{' '}
                                        <b className="tabular-nums">{batchProgress.sent ?? 0}</b>
                                    </span>
                                    <span className="text-red-600 dark:text-red-400">
                                        ✕ Failed:{' '}
                                        <b className="tabular-nums">{batchProgress.failed ?? 0}</b>
                                    </span>
                                    <span className="text-gray-500 dark:text-gray-400">
                                        ⤼ Skipped:{' '}
                                        <b className="tabular-nums">{batchProgress.skipped ?? 0}</b>
                                    </span>
                                    {batchProgress.lastError && (
                                        <span
                                            className="text-red-500 truncate max-w-full"
                                            title={batchProgress.lastError}
                                        >
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        onClick={() => {
                                            ...
                                            toast.info(
                                                `🔍 Debug: ${customers.length} customers loaded, ${isLoading ? 'Loading...' : 'Ready'}`,
                                            );
                                        }}
                                        variant="outline"
                                        size="sm"
                                    ...
                                    <Button
                                        onClick={() => {
                                            void (
                                                /* console.log */ ((..._args) => {})(
                                                    '🔄 FORCE REFRESH triggered - clearing state and refetching...',
                                                )
                                            );
                                            setCustomers([]);
                                            setIsLoading(true);
