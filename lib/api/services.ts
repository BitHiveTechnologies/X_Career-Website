...
export class JobService {
    ...
    static async getJobs(params?: JobSearchParams): Promise<ApiResponse<JobsResponse>> {
        void (
            /* console.log */ ((..._args) => {})(
                '🔍 JobService.getJobs called with params:',
                params,
            )
        );
        void (
            /* console.log */ ((..._args) => {})(
                '🔍 API_ENDPOINTS.JOBS.ALL:',
                API_ENDPOINTS.JOBS.ALL,
            )
        );
    ...
    static async searchJobs(
        query: string,
        params?: Omit<JobSearchParams, 'search'>,
    ...
    static transformToFrontendJob(job: Job): FrontendJob {
        ...
        const getCompanyType = (companyName: string): 'Startup' | 'MNC' | 'Product' | 'Service' => {
            ...
            if (
                [
                    ...
                    'labs',
                    'ventures',
                ].some((startup) => name.includes(startup))
            ) {
                return 'Startup';
            }
            ...
            // Service companies
            if (
                ['consulting', 'services', 'solutions', 'advisory', 'urban company', 'nykaa'].some(
        ...
        const getExperienceLevel = (title: string, description: string = ''): string => {
            ...
            if (
                [
                    ...
                    '7+',
                    '10+',
                ].some((senior) => combinedText.includes(senior))
            ) {
                return '5+ years';
            }
            ...
            // Mid level keywords
            if (
                ['mid', 'intermediate', 'experienced', '3-5', '4-5', '3-7'].some((mid) =>
                    combinedText.includes(mid),
                )
            ) {
                return '3-5 years';
            }
            ...
            if (
                ['junior', 'entry', 'associate', '1-3', '2-3', '1-2', '2-4'].some((junior) =>
            ...
            if (
                ['intern', 'internship', 'fresher', 'trainee', 'entry level', '0-1', '0-2'].some(
                    (intern) => combinedText.includes(intern),
                )
            ) {
                return '0-1 years';
            }
        ...
        const getEmploymentType = (type: string, title: string): string => {
            ...
            if (
                ['contract', 'freelance', 'consultant'].some((contract) =>
        ...
        return {
            ...
            industry: ['Technology', 'Fintech', 'E-commerce', 'Healthcare', 'Education'][
                Math.floor(Math.random() * 5)
            ],
            benefits: [
                'Health Insurance',
...
export class MatchingService {
     ...
     */
    static async getMatchingUsersForJob(
        jobId: string,
        params?: {
            page?: number;
            limit?: number;
...
export class AdminService {
    ...
    static async getAllPayments(params?: {
        ...
        void (
            /* console.log */ ((..._args) => {})(
                '🔍 AdminService.getAllPayments called with params:',
                params,
            )
        );
    ...
    static async getAllCustomers(params?: {
        ...
        void (
            /* console.log */ ((..._args) => {})(
                '🔍 AdminService.getAllCustomers called with params:',
                params,
            )
        );
...
export class JobAlertService {
     ...
     */
    static async triggerSchedulerTask(
...
export const { register, login, adminLogin, getMe, verifyToken } = AuthService;
...
export const { createOrder, verifyPayment, getPaymentHistory } = PaymentService;
...
export const { getDashboardStats, getUserAnalytics, getJobAnalytics, getSystemHealth, getUsers } =
    AdminService;

export const { testEmailConnection, sendWelcomeEmail, sendJobAlertEmail, getEmailQueueStatus } =
    NotificationService;
...
export const { checkHealth, getApiDocs } = HealthService;

export const { getMyResume, saveResume, getPublicResume } = ResumeService;
